 ...
 *    `@platform-blocks/ui` entry (~2 MB single-file bundle), so demo code is
 *    rewritten to import from the trimmed `@platform-blocks/ui/snack` entry.
...
const UI_PACKAGE = '@platform-blocks/ui';
 ...
 *
 * - `@platform-blocks/ui` and repo-relative paths that point back into the
 ...
 * the provider stack, the scrolling screen, and the Platform Blocks footer. A
...
export function buildSnackUrl({
  ...
  const url = snackUrlFor(
    `${component} — ${title}`,
    `${component} demo from platform-blocks.com`,
    files,
    platform
  );
...
function buildBundleAppFile(
  ...
  const omittedNote = skippedCount === 1
    ? '1 more demo on platform-blocks.com — not included in this Snack.'
    : `${skippedCount} more demos on platform-blocks.com — not included in this Snack.`;
...
export default function App() {
  return (
    <Shell
      ...
      subtitle=${jsString(`platform-blocks.com/components/${component}`)}${note}
    >
...
export function buildComponentSnackUrl(
  ...
  for (let count = demos.length; count > 0; count--) {
    ...
    const url = snackUrlFor(
      ...
      `${component} demos from platform-blocks.com`,
      bundle.files,
      platform
    );
