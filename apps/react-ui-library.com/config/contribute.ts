...
export const CONTRIBUTE_TITLE = 'Contributing to React UI Library';
...
export const CONTRIBUTE_REPO_LAYOUT: ContributeRepoPath[] = [
  {
    path: 'packages/ui',
    description: 'The component library published as `@platform-blocks/react-ui-library` — 100+ components, hooks, and the theming system',
  },
  {
  ...
  {
    path: 'apps/react-ui-library.com',
...
export const CONTRIBUTE_SECTIONS: ContributeSection[] = [
  {
    ...
    lead: 'React UI Library is one npm workspace. Install from the root — the workspaces are linked, so the docs site runs against your local packages rather than the published ones.',
    snippets: [
      {
...
npm run ui:lint      # eslint`,
  ...
  },
  {
    ...
    items: [
      ...
      'Add a row to `apps/react-ui-library.com/config/coreComponents.ts`.',
  ...
  },
  {
    ...
    lead: 'Guide pages keep their copy in JSX-free modules under `apps/react-ui-library.com/config/` — `gettingStarted.ts`, `templates.ts`, `faq.ts`, and this page\'s `contribute.ts` — so `scripts/generate-llms.ts` can render the same source into `llms.txt` for language models. A new page touches four files:',
    items: [
  ...
  },
  {
    key: 'releases',
    title: 'Releases',
    lead: 'Maintainers run `npm run release`, which verifies both packages (exports, lint, tests, build) and publishes `@platform-blocks/react-ui-library` and `@platform-blocks/charts` to npm. Contributors never need to bump a version — say what the change is in the pull request and it lands in the next release.',
  },
];
