...
export { SITE_URL };
export const SITE_NAME = 'React UI Library';
...
const DEFAULT_DESCRIPTION =
  'Build polished, accessible interfaces for iOS, Android, and Web with 100+ React Native components, powerful hooks, theming, and production-ready charts.';

/** Metadata for the static (non-parameterized) routes, keyed by pathname. */
const STATIC_ROUTES: Record<string, RouteMeta> = {
  '/': {
    title: 'React UI Library — One UI System for Every Screen',
    description: DEFAULT_DESCRIPTION,
  },
  '/getting-started': {
    title: 'Getting Started',
    description:
      'Install @platform-blocks/react-ui-library, wire up the provider, and render your first themeable, cross-platform component on iOS, Android, and Web.',
  },
  '/extensions': {
    title: 'Extensions',
    description:
      'Packages that build on React UI Library — official and community extensions, plus a batteries-included template for building and publishing your own.',
  },
  '/examples': {
    title: 'Examples',
    description:
      'Complete screens built from React UI Library components — login, settings, dashboards, and more. Open fullscreen, then copy the source into your app.',
  },
  '/examples/login': {
    title: 'Login Screen Example',
    description:
      'A complete React Native sign-in screen built with React UI Library — inputs with inline validation, social sign-in buttons, and a remember-me control.',
  },
  '/examples/settings': {
    title: 'Settings Screen Example',
    description:
      'A grouped React Native preferences screen built with React UI Library — switch rows, a live theme-mode selector, and a profile header.',
  },
  '/examples/dashboard': {
    title: 'Stats Dashboard Example',
    description:
      'A responsive KPI dashboard built with React UI Library — stat tiles with trend sparklines from @platform-blocks/charts.',
  },
  '/components': {
    title: 'Components',
    description:
  ...
  '/accessibility': {
    title: 'Accessibility',
    description:
      'How React UI Library builds in keyboard navigation, screen-reader support, focus management, and semantic roles across every component.',
  },
  '/contribute': {
    title: 'Contributing to React UI Library',
    description:
      'Contribute to React UI Library — the monorepo layout, local setup, and the workflow for adding a component, a demo, or a documentation page.',
  },
  '/localization': {
    title: 'Localization',
    description:
      'Internationalize your app with React UI Library — RTL support, locale-aware formatting, and translatable component copy.',
  },
  '/llms': {
    title: 'LLM Documentation',
    description:
      'React UI Library documentation as Markdown for language models — an llms.txt index, a full-text bundle, and a standalone Markdown file for every component, chart, hook, and guide.',
  },
  '/faq': {
    title: 'FAQ',
    description:
      'Frequently asked questions about React UI Library — licensing, platform support, theming, and how it compares to other React Native UI libraries.',
  },
};
...
function resolveRouteMetaUnclamped(rawPathname: string): RouteMeta {
  ...
  if (segments.length === 2) {
    ...
    if (section === 'components') {
      ...
      const fallback = `A themeable, accessible, cross-platform React Native component from React UI Library.`;
      return {
    ...
    if (section === 'charts') {
      ...
      return {
        ...
        description: withMinimumContext(
          ...
          `${label} — a cross-platform chart for React Native and Web from React UI Library Charts.`
        ),
      };
    }
    ...
    if (section === 'hooks') {
      ...
      return {
        ...
        description: withMinimumContext(
          meta?.description ? stripMarkdown(leadParagraph(meta.description)) : '',
          `${label} — a React hook for building cross-platform apps with React UI Library.`
        ),
      };
    }
    ...
    if (section === 'examples') {
      ...
      return {
        title: `${label} Example`,
        description: `${label} — a complete mini-app example built with React UI Library components.`,
      };
    }
  }
