...
export interface ExtensionEntry {
  ...
  name: string;
  description: string;
  /** Maintained by the React UI Library team. */
  official: boolean;
  npmUrl: string;
  repoUrl: string;
}
...
export const EXTENSIONS_SUBTITLE =
  'Packages that build on React UI Library — maintained by the core team and by the community.';
...
export const EXTENSION_CONTRACT: ExtensionContractPoint[] = [
  {
    ...
    detail:
      'Colors, spacing, radii, and typography come from the app\'s React UI Library theme, so an extension changes with the theme instead of shipping a palette of its own — including light and dark.',
  },
  {
...
export const EXTENSIONS_PUBLISH_STEPS: ExtensionContractPoint[] = [
  ...
  {
    title: 'Keep @platform-blocks/react-ui-library a peer dependency',
    detail:
...
export const EXTENSIONS: ExtensionEntry[] = [
  {
    ...
    repoUrl: 'https://github.com/platform-blocks/react-ui-library/tree/main/packages/charts',
  },
];
