...
**The root `useHaptics` export now resolves to the documented hook.** Two different hooks were named `useHaptics`: the one documented at `/hooks/useHaptics` and used internally by `Button`, `IconButton`, and `Toast` (`impactPressIn` / `impactPressOut` / `notifySuccess` / `notifyWarning` / `notifyError` / `selection`), and the sound system's `triggerHaptic` wrapper. The package root only ever exported the latter, so `import { useHaptics } from '@platform-blocks/react-ui-library'` returned a hook whose API did not match its own documentation. The root name now belongs to the documented hook; the sound-system one is exported as **`useSoundHaptics`**, matching the module's existing `useMockHaptics` naming.
